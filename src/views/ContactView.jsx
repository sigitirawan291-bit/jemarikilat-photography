    ...
    <div className="w-full pt-28 pb-24 px-6 lg:px-16 bg-[#FAF8F5] text-[#1A1A1A] min-h-screen font-sans">
      <div className="max-w-[1440px] mx-auto space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8 bg-white p-8 lg:p-10 border border-[#E5E0D8] rounded-xs shadow-xs">
            <div>
              <span className="font-sans text-[10px] tracking-[0.3em] text-[#C5A880] uppercase block mb-1 font-bold">STUDIO HEADQUARTERS</span>
              <h2 className="font-serif text-2xl text-[#1A1A1A] font-normal mb-3">JEMARI KILAT Medan</h2>
              <p className="font-sans text-xs text-[#666158] leading-relaxed font-light">
                Kota Medan, Sumatra Utara, Indonesia
              </p>
            </div>

            <div className="space-y-4 pt-6 border-t border-[#E5E0D8] font-sans text-xs">
              ...
              <div className="flex items-center gap-3">
                ...
                <a 
                  ...
                  className="hover:underline text-[#1A1A1A] font-semibold flex items-center gap-1.5"
                >
            ...
            <div className="pt-6 border-t border-[#E5E0D8] space-y-3">
              <span className="font-sans text-[10px] tracking-[0.25em] text-[#C5A880] uppercase block font-bold">INSTANT RESERVATION</span>
              <button
                onClick={onOpenBooking}
                className="w-full bg-[#1A1A1A] text-white font-sans text-xs tracking-[0.2em] py-3.5 text-center uppercase font-bold hover:bg-[#C5A880] hover:text-black transition-all shadow-xs border border-[#1A1A1A]"
              >
                OPEN BOOKING FORM
              </button>
            </div>
          ...
          <div className="lg:col-span-7 bg-white border border-[#E5E0D8] rounded-xs overflow-hidden flex flex-col h-[600px] shadow-xs">
            
            {/* Console Header */}
            <div className="bg-[#1A1A1A] text-white p-4 lg:p-6 flex justify-between items-center border-b border-[#1A1A1A]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#FAF8F5] text-[#C5A880] border border-[#C5A880] flex items-center justify-center font-serif font-bold text-sm">
                  JK
                </div>
                <div>
                  <h3 className="font-serif text-lg font-normal text-white">Sigit Irawan (Art Director)</h3>
                  <span className="font-sans text-[10px] text-[#C5A880] tracking-wider uppercase block font-mono">
            ...
            <div className="p-4 bg-[#FAF8F5] border-b border-[#E5E0D8] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs font-sans">
              <div className="flex items-center gap-2">
                <Phone className="w-4 h-4 text-emerald-600" />
                <span className="font-bold text-[#1A1A1A]">No. WhatsApp Anda (Wajib):</span>
              </div>
              <input
                type="tel"
                value={userWa}
                ...
                className="w-full sm:w-64 bg-white px-3 py-2 border border-[#E5E0D8] text-xs text-[#1A1A1A] focus:outline-none focus:border-[#C5A880] placeholder:text-[#7A756C] rounded-xs"
                required
              />
            </div>

            {/* Message Area */}
            <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-[#FAF8F5] font-sans text-xs">
              {messages.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {isAdmin && m.waUrl && (
                        <button
                          ...
                          className="mt-3 w-full sm:w-auto py-2.5 px-4 bg-emerald-700 hover:bg-emerald-800 text-white font-sans text-xs font-bold tracking-wider uppercase flex items-center justify-center gap-2 transition-colors shadow-xs rounded-xs"
                        >
                    ...
                    <span className="text-[9px] text-[#7A756C] mt-1 px-1 font-mono">{m.time}</span>
                  </div>
                );
              })}

              {isTyping && (
                <div className="text-[#C5A880] text-[10px] italic">
            ...
            {/* Input Form */}
            <form onSubmit={handleSend} className="p-4 bg-white border-t border-[#E5E0D8] flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <input
                type="text"
                value={inputMsg}
                onChange={(e) => setInputMsg(e.target.value)}
                placeholder="Tuliskan pertanyaan atau rencana tanggal acara Anda..."
                className="flex-1 bg-transparent text-xs text-[#1A1A1A] input-underline"
              />
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="submit"
                  title="Kirim pesan ke konsol chat"
                  className="bg-[#1A1A1A] text-white font-sans text-xs tracking-[0.15em] px-5 py-3 uppercase font-bold hover:bg-[#C5A880] hover:text-black transition-colors flex items-center justify-center gap-2 whitespace-nowrap rounded-xs shadow-xs"
                >
                ...
                <button
                  ...
                  className="bg-emerald-700 text-white font-sans text-xs tracking-[0.1em] px-4 py-3 uppercase font-bold hover:bg-emerald-800 transition-colors flex items-center justify-center gap-1.5 whitespace-nowrap rounded-xs shadow-xs"
                >
